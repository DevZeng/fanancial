...
//        $proxy = WeChatUser::find($id);
    ...
    public function myAgents()
    {
        ...
        foreach ($agents as $agent){
            ...
            $agent->loanUserCount = count(Loan::where('proxy_id','=',$uid)->groupBy('user_id')->pluck('user_id'));
...
//        dd($permissions);
    ...
    public function UserBill(Request $post)
    {
        $user_id = $post->user_id;
        $page = $post->page ? $post->page : 1;
        $limit = $post->limit ? $post->limit : 10;
        $loans = Loan::where('proxy_id','=',$user_id)->limit($limit)->offset(($page-1)*$limit)->orderBy('id','DESC')->get();
        if (!empty($loans)){
